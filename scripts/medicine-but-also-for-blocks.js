//Took script from z0mbiesrock/Diamond-ore
const archangelHeal = newEffect(15, e => {
  Draw.color(Color.valueOf("#84f491"), Color.valueOf("#62ae7f"), e.fin());
...
const archangelHealFull = newEffect(18, e => {
  Draw.color(Color.valueOf("#84f491"), Color.valueOf("#62ae7f"), e.fin());
    const g = new Floatc2({get(x, y){
...
const archangelUnit = new JavaAdapter(UnitType, {}, "medic-mk2",  prov(() => new JavaAdapter(HoverUnit, {
  draw(){
    this.super$draw();
    try{
      if(this.targetIsBeingRepaired == true){
        ...
        Drawf.laser(Core.atlas.find("infinitodustry-medibeam"),Core.atlas.find("infinitodustry-medibeam-end"), this.x, this.y, this.healTarget.x, this.healTarget.y, this.beamstrength);
      }
    }
    catch(wait){
  ...
  getPowerCellRegion(){
        return Core.atlas.find("infinitodustry-medic-mk2-cell");
    },
  move(x, y){
    try{
      ...
      if(this.healTarget != null){
        ...
        if (this.healTarget.dst(this) <= 56){
          ...
          if (this.healTarget.health == this.healTarget.maxHealth()){
            Effects.effect(archangelHealFull, this.healTarget);
          }
          else if (Mathf.chance(0.075)){
            Effects.effect(archangelHeal, this.healTarget);
          }
        }
        if(this.targetIsBeingRepaired == true){
